...
imm_handle create_immersitech_processor() {
	...
	imm_handle handle = imm_initialize_library(IMM_LICENSE_PATH, IMM_ROOM_LAYOUT_PATH, IMM_WEBSOCKET_PATH, config, &error_code);
...
imm_error_code immersitech_set_state(imm_handle handle, imm_audio_control control, int value) {
	imm_set_all_participants_state(handle, IMM_PROCESSOR_ROOM_ID, control, value);
}
